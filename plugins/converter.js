...
const Telegraph = async (path) => {
  ...
  if (!mime || !mime.startsWith("image/")) {
    return reply(" *Invalid format!* Reply with valid image media.");
  }

  let media = await q.download();
  let filePath = "./" + new Date().getTime() + ".jpg";

  fs.writeFileSync(filePath, media);

  reply("🔄 *Uploading images to the Telegraph... Please wait a moment.*");
  ...
  try {
    ...
    reply(
      `✅ *Image uploaded successfully!*\n\n🌐 *Link Telegraph:*\n${uploadedLinks}\n\n📤 *Use this link to share images easily!*`
    );
  } catch (err) {
    console.error(err);
