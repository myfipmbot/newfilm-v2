...
if (!isMe) return await reply(BOTOW)	
if (q == 'on') {
  if ( config.AUTO_REACT_STATUS == 'true') return reply('already Autoreactstatus  is on ')
  await input_set('AUTO_REACT_STATUS' , 'true')
  return reply('Autoreactstatus turned on')
  }
if ( q == 'off' ) {
   if ( config.AUTO_REACT_STATUS !== 'true') return reply('already Autoreactstatus is off')
  await input_set('AUTO_REACT_STATUS' , 'false')
  return reply('Autoreactstatus turned off')
}
...
try{
if (!isMe) return await reply(BOTOW)	
let dat = `👨‍💻 ᴋᴀᴠɪ ᴇxᴇ ᴠ1 ᴄʀᴇᴀᴛᴇᴅ ʙʏ ᴋᴀᴠɪ ʙᴏʏ 👨‍💻
    ...
    },
{
 ...
 {
	title: "━━━━━━━━━━━━━━━━━━\n\n`[8] 𝗔𝗨𝗧𝗢 𝗥𝗘𝗔𝗖𝗧 𝗦𝗧𝗔𝗧𝗨𝗦 𝗦𝗘𝗧𝗧𝗜𝗡𝗚`",
	rows: [
  {title: "    28.1", rowId: prefix + 'autoreactstatus on ' + q , description: 'To Enable Auto react status On 🔑'}, 
  {title: "    28.2", rowId: prefix + 'autoreactstatus off ' + q , description: 'To Disable Auto react status Off 🔒'},	
]
}
]
  const listMessage = {
